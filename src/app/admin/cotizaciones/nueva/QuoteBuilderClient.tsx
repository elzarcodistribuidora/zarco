...
export function QuoteBuilderClient({ productos }: { productos: any[] }) {
  ...
  const handleItemUpdate = (index: number, updates: Partial<Item>) => {
    const newItems = [...items];
  ...
  return (
    <div className="admin-enter mx-auto max-w-4xl space-y-6">
      <datalist id="productos-list">
        {productos.map(p => (
          <option key={p.codigo} value={p.nombre_web} />
        ))}
      </datalist>
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex flex-wrap items-start gap-4 rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                <div className="flex-1 min-w-[200px]">
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">Descripción o Producto</label>
                  <input
                    type="text"
                    list="productos-list"
                    required
                    ...
                    onChange={(e) => {
                      ...
                      const matched = productos.find((p) => p.nombre_web === val);
                      handleItemUpdate(idx, {
                        descripcion: val,
                        ...(matched ? { precio_unitario: matched.precio_final || 0 } : {}),
                      });
                    }}
                ...
                <div className="w-24">
                  ...
                  <input
                    ...
                    required
                    value={item.cantidad}
                    onChange={(e) => handleItemUpdate(idx, { cantidad: Number(e.target.value) })}
                    className="mt-1 block w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-[#A81200] focus:outline-none"
                  />
                </div>
                <div className="w-32">
                  ...
                  <input
                    ...
                    onChange={(e) => handleItemUpdate(idx, { precio_unitario: Number(e.target.value) })}
                    className="mt-1 block w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-[#A81200] focus:outline-none"
                  />
                </div>
